...
test("combat transitions recheck all volatile restrictions and recover after combat", function()
  ...
  equal(state.cooldowns["neutral.strike"].usable, true)
  data.combat = false
  engine:HandleEvent("PLAYER_REGEN_ENABLED")
  equal(engine:GetSnapshot().cooldowns["neutral.strike"].duration, 10)
end)

test("public usability survives restricted cooldown without stale timings or false readiness", function()
  local engine, data, _, env = fixture()
  engine:HandleEvent("PLAYER_ENTERING_WORLD")
  ...
  equal(state.capabilities["cooldowns.neutral.strike.usable"], "ADDON_AVAILABLE")
  local reader = namespace.StateReader.Create(state, namespace.CompatFactory.Create(env).State)
  local ready, reason = reader:IsReady({id="neutral.strike"})
  equal(ready, false); equal(reason, "COOLDOWN_UNAVAILABLE")
  data.usable = false; engine:HandleEvent("SPELL_UPDATE_USABLE")
  equal(engine:GetSnapshot().cooldowns["neutral.strike"].usable, false)
  data.usable = SECRET; engine:HandleEvent("SPELL_UPDATE_USABLE")
  ...
  env.C_Secrets.ShouldSpellCooldownBeSecret = function() return false end
  data.usable = true; engine:HandleEvent("PLAYER_REGEN_ENABLED")
  equal(engine:GetSnapshot().cooldowns["neutral.strike"].duration, 10)
end)

test("public charges survive failed cooldown normalization with only child authorization", function()
  local engine, data, _, env = fixture()
  ...
  equal(state.cooldowns["neutral.strike"].duration, nil)
  equal(state.capabilities["cooldowns.neutral.strike"], "CONDITIONALLY_SECRET")
  equal(state.capabilities["cooldowns.neutral.strike.charges"], "ADDON_AVAILABLE")
  local reader = namespace.StateReader.Create(state, namespace.CompatFactory.Create(env).State)
  equal(reader:Read({"cooldowns","neutral.strike","ready"}), nil)
end)
...
test("bootstrap registers and forwards state events only after this addon loads", function()
  ...
  namespace.Profiles = { Start = function() end }
